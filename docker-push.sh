...
then
    ...
    then
        # users
        docker build $USERS_REPO -t $USERS:$COMMIT -f Dockerfile-$TRAVIS_BRANCH
        ...
        # client
        docker build $CLIENT_REPO -t $CLIENT:$COMMIT -f Dockerfile-$TRAVIS_BRANCH --build arg REACT_APP_USERS_SERVICE_URL=TBD
        docker tag $CLIENT:$COMMIT $REPO/$CLIENT:$TAG
        docker push $REPO/$CLIENT:$TAG
        # swagger
        docker build $SWAGGER_REPO -t $SWAGGER:$COMMIT -f Dockerfile-$TRAVIS_BRANCH $SWAGGER_DIR
